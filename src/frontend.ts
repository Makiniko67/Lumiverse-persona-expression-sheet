...
// ──────────────────────────────────────────────────────────────────────────
// Types (mirror the backend's shapes — kept local since the two modules
// run in separate bundles and only talk over postMessage).
// ──────────────────────────────────────────────────────────────────────────
...
interface StateMessage {
  ...
  data: PersonaExpressionData | null
  widgetSize: number
  scanCharacterMessages: boolean
  permissionsMissing: boolean
}

interface ActiveChangedMessage {
  type: 'active_changed'
  personaId: string
  label: string
  slot: ExpressionSlot
...
type BackendMessage = StateMessage | ActiveChangedMessage | RefetchMessage
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  let widget: ReturnType<typeof ctx.ui.createFloatWidget> | null = null
  let widgetImg: HTMLImageElement | null = null
  ...
  const removeStyle = ctx.dom.addStyle(`
    ...
    .pes-widget {
      ...
      border-radius: 12px;
      overflow: hidden;
      background: var(--lumiverse-fill-subtle, rgba(20, 20, 22, 0.85));
      border: 1px solid var(--lumiverse-border, #333);
      display: flex;
      align-items: center;
      justify-content: center;
  ...
  function applyWidgetSize(size: number) {
    if (!widget) return
  ...
  function ensureWidget(size: number) {
    if (widget && currentWidgetSize === size) return widget
    if (widget) {
  ...
  function updateWidget(url: string | null, size: number) {
    const w = ensureWidget(size)
    if (!w || !widgetImg) return
    if (url) {
      widgetImg.src = url
  ...
  async function pickAndUpload(label: string) {
    let files
    try {
  ...
  async function showCardMenu(label: string, hasSlot: boolean, evt: MouseEvent) {
    const items = hasSlot
      ? [
    ...
    if (selectedKey === 'replace') {
      ...
      if (confirmed) removeLabel(label)
    }
  }
  ...
  function buildCard(label: string, slot: ExpressionSlot | undefined, isActive: boolean): HTMLElement {
