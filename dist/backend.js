...
async function runDetection(content, userId, source) {
    const personaId = await getActivePersonaId(userId);
    if (!personaId)
        return;
    const store = await loadStore(userId);
...
spindle.on('MESSAGE_SENT', async (payload) => {
    ...
    if (!message || typeof message.content !== 'string' || message.role !== 'user')
        return;
    if (!lastKnownUserId)
...
spindle.on('GENERATION_ENDED', async (payload) => {
    ...
    if (!store.scanCharacterMessages)
        return;
    ...
    const content = payload?.message?.content ?? payload?.content ?? payload?.text ?? payload?.message?.text ?? payload?.result?.content;
